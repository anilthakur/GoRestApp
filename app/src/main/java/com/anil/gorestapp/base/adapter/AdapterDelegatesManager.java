 ...
 * <ul>
 * <li> {@link #getItemViewType(List, int)} <Object>, int)}: Must be called from {@link
...
public class AdapterDelegatesManager<T> {
    ...
    /**
     * Used internally for {@link #onBindViewHolder(List, int, RecyclerView.ViewHolder)} as empty
     * payload parameter
     */
    ...
    public int getItemViewType(@NonNull List<T> items, int position) {
        ...
        throw new NullPointerException(
                "No AdapterDelegate added that matches position = " + position + "" +
                        "and type = " + items.get(position).getClass().getCanonicalName() + " in data source." +
                        "Please add a delegate that is responsible for this item type.");
    }
    ...
    public void onBindViewHolder(@NonNull List<T> items, int position,
        ...
        delegate.onBindViewHolder(items, position, viewHolder, payloads == null ? Collections.emptyList() : payloads);
    }
